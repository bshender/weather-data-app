                    ...
                    return (
                        <Card className={isLoading ? Classes.SKELETON : ""}>
                            ...
                            <ActionLine iconName="globe" url={mapurl} text={<div>
                                ...
                                {address?.addressLocality}<span>, </span>{address?.addressRegion}<span> </span>{address?.postalCode}<br />
                            </div>} />
                            <ActionLine iconName="envelope" url={`mailto:${officeData?.email}`} text={<span>{officeData?.email}</span>} />
                            <ActionLine iconName="phone" url={`tel:${officeData?.email}`} text={<span>{officeData?.telephone}</span>} />
                        </Card>
                    )
                }
            }
